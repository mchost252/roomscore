...
export default function ChatScreen() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // ALWAYS stop loading — never stuck on spinner
  ...
  const performClear = useCallback(() => {
    if (user?.id && friendId) {
      sqliteService.deleteConversationMessages(user.id, friendId).then(async () => {
        setMessages([]);
        // Also update conversation to clear last_message so messages screen shows "Start new conversation"
        ...
        if (conv) {
          ...
          messageService.notifyConversationUpdate();
        }
  ...
  const renderMessage = useCallback(({ item, index }: { item: LocalDirectMessage; index: number }) => {
    // Only animate messages that are newly added (after initial load)
    const isNew = index >= prevMessageCountRef.current;
    const bubble = (
      <ChatBubble
        message={item}
          ...
          {loading ? (
            ...
            <Animated.View style={[styles.flex, listAnimatedStyle]}>
              <FlashList
                ...
                // FIX: Load older messages at the TOP of the list (not bottom)
                // onEndReached fires at the bottom — we want onStartReached behavior
                // Since FlashList doesn't support onStartReached, we detect scroll near top
                onEndReachedThreshold={0.3}
                onContentSizeChange={() => {
                  // Only auto-scroll to bottom on initial load or when new message is sent
                  if (messages.length > 0 && !loading && !hasScrolledToBottom.current) {
                    setTimeout(() => {
                ...
                ListHeaderComponent={
                  <>
                    {/* Load more indicator at TOP (non-layout-shifting) */}
                    {loadingMore && (
                      <View style={styles.loadingMoreTop}>
                        <ActivityIndicator size="small" color={accentColor} />
                      </View>
                    )}
                    {/* Load more trigger — tap to load older */}
